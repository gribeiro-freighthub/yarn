...
export default class NpmRegistry extends Registry {
  ...
  isRequestToRegistry(requestUrl: string, registryUrl: string): boolean {
    ...
    const requestToYarn =
      YARN_REGISTRY.includes(request.host) &&
      DEFAULT_REGISTRY.includes(registry.host)
    ...
    const customHostSuffixInUse =
      typeof customHostSuffix === 'string' &&
      request.host.endsWith(customHostSuffix)

    return (
      (requestToRegistryHost || requestToYarn) &&
      (requestToRegistryPath || customHostSuffixInUse)
    )
  }
  ...
  async request(
    ...
    const packageIdent =
      (packageName && NpmRegistry.escapeName(packageName)) || pathname
    ...
    const headers = {
      ...
      'Keep-Alive': 'timeout=200, max=10000',
      ...opts.headers,
    }

    const isToRegistry =
      this.isRequestToRegistry(requestUrl, registry) ||
      this.requestNeedsAuth(requestUrl)

    // this.token must be checked to account for publish requests on non-scoped packages
    if (
      this.token ||
      (isToRegistry && (alwaysAuth || this.isScopedPackage(packageIdent)))
    ) {
  ...
  async checkOutdated(
    ...
    const wantedPkg = await NpmResolver.findVersionInRegistryResponse(
      config,
      escapedName,
      range,
      req
  ...
  async getPossibleConfigLocations(
    ...
    if (this.enableDefaultRc) {
      ...
      while (foldersFromRootToCwd.length > 1) {
        possibles.push([
          false,
          path.join(foldersFromRootToCwd.join(path.sep), localfile),
        ])
  ...
  getRegistry(packageIdent: string): string {
    ...
    if (packageIdent.match(REGEX_REGISTRY_PREFIX)) {
      ...
      const registry = availableRegistries.find(registry =>
        packageIdent.startsWith(registry)
      )
      if (registry) {
  ...
  getAuthByRegistry(registry: string): string {
    ...
    if (username && password) {
      ...
      return (
        'Basic ' + Buffer.from(String(username) + ':' + pw).toString('base64')
      )
    }
